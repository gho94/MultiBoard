...
public class BoardController {
	...
	public String getListByCategory(@PathVariable int categoryId, @PathVariable int page, HttpSession session, Model model) {
		...
		List<Board> boards = boardService.selectArticleListByCategory(categoryId, page);
	...
	public String writeArticle(@PathVariable int categoryId, HttpSession session, Model model) {
		...
		List<BoardCategory> categories = categoryService.selectAllCategory();
	...
	public String writeArticle(Board board, BindingResult result, String csrfToken, HttpSession session, RedirectAttributes redirectAttributes) {
		logger.info("/board/write : " + board.toString() + csrfToken);
		String sesstionToken = (String) session.getAttribute("csrfToken");
		
		if (csrfToken == null || csrfToken.equals(sesstionToken)) {
	...
	public String replyArticle(Board board, RedirectAttributes redirectAttributes, HttpSession session) {
		...
		if (session.getAttribute("page") != null) {
			...
			return "redirect:/board/cate/" + board.getCategoryId();
		}
	}
	...
	public String updateArticle(Board board, RedirectAttributes redirectAttributes) {
		...
		if (!board.getPassword().equals(dbPassword)) {
			redirectAttributes.addFlashAttribute("passwordError", "게시글 비밀번호가 다릅니다.");
	...
	public String deleteArticle(Board board, HttpSession session, RedirectAttributes redirectAttributes) {
		try {
			...
			if (dbpw.equals(board.getPassword())) {
				...
				redirectAttributes.addFlashAttribute("message", "WRONG_PASSWORD_NOT_DELETED");
	...
	public String search(@RequestParam(required=false, defaultValue="") String keyword, @PathVariable int page, HttpSession session, Model model) {
		try {
			List<Board> boards = boardService.searchListByContentKeyword(keyword, page);
			model.addAttribute("boardList", boards);
